...
class PPCDetector(object):
    ...
    def _compute_qq(self) -> None:
        ...
        self._obs_cum_rate = (self._obs_counts[self._mask] -
                              self._scale_factor *
                              self._obs_background[self._mask]
                              ).cumsum()

        self._ppc_cum_rate = (self._ppc_counts[:,self._mask] -
                              self._scale_factor *
                              self._ppc_background[:,self._mask]
                              ).cumsum(axis=1)
